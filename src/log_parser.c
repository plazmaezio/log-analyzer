...
int parse_nginx_error(const char *line, NginxErrorEntry *entry)
{
    ...
    {
        level_start++;
        if (strncmp(level_start, "debug", 5) == 0)
            entry->level = NGINX_DEBUG;
        else if (strncmp(level_start, "info", 4) == 0)
            entry->level = NGINX_INFO;
        else if (strncmp(level_start, "notice", 6) == 0)
            entry->level = NGINX_NOTICE;
        else if (strncmp(level_start, "warn", 4) == 0)
            entry->level = NGINX_WARN;
        else if (strncmp(level_start, "error", 5) == 0)
            entry->level = NGINX_ERROR;
        else if (strncmp(level_start, "crit", 4) == 0)
            entry->level = NGINX_CRIT;
        else if (strncmp(level_start, "alert", 5) == 0)
            entry->level = NGINX_ALERT;
        else if (strncmp(level_start, "emerg", 5) == 0)
            entry->level = NGINX_EMERG;
    }
